...
const API = {
  ...
  // Import and parse entire Google Sheet CSV into products database (Columns A to R)
  importProductsFromCSV(csvText) {
    if (!csvText || typeof csvText !== 'string') return { success: false, message: 'CSV ফাইল খালি।' };
    
    // RFC-compliant CSV parser handling multiline descriptions and escaped quotes
    const rows = [];
    let currentRow = [];
    let currentCell = '';
    let inQuotes = false;
    ...
    if (rows.length <= 1) return { success: false, message: 'CSV ফাইলে কোনো প্রোডাক্ট ডেটা পাওয়া যায়নি।' };
    ...
    if (parsedProducts.length > 0) {
      this.setStorage(this.STORAGE_KEYS.PRODUCTS, parsedProducts);
      this._sheetLoaded = true;
      console.log(`[CSV Import] Successfully parsed and loaded ${parsedProducts.length} products from CSV.`);
      window.dispatchEvent(new CustomEvent('dcbd_products_synced', { detail: parsedProducts }));
      return { success: true, count: parsedProducts.length, products: parsedProducts };
    }
    return { success: false, message: 'সঠিক ফরম্যাটে কোনো প্রোডাক্ট পাওয়া যায়নি।' };
  },
  ...
  initSeedData() {
    ...
    const hasFabricated = Array.isArray(prods) && prods.some(p => p.sku && p.sku.startsWith('DCB-PRD-'));
    const hasBrokenImg = Array.isArray(prods) && prods.some(p => p.primaryImage && (p.primaryImage.indexOf('\\_') !== -1 || p.primaryImage.indexOf('\_') !== -1));
    if (!Array.isArray(prods) || prods.length === 0 || hasOldDummy || hasFabricated || hasBrokenImg) {
